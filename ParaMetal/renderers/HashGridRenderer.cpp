...
bool HashGridRenderer::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr,
        ...
        std::cerr << "HashGridRenderer: Failed to create descriptor set layout" << std::endl;
        return false;
    }
...
bool HashGridRenderer::createDescriptorPool(uint32_t maxFramesInFlight) {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr,
        ...
        std::cerr << "HashGridRenderer: Failed to create descriptor pool" << std::endl;
        return false;
    }
...
bool HashGridRenderer::createPipeline(VkRenderPass renderPass, uint32_t subpass) {
    std::vector<char> vertCode;
    std::vector<char> fragCode;
    if (!readFile("shaders/hash_grid_vis_vert.spv", vertCode) ||
        !readFile("shaders/hash_grid_vis_frag.spv", fragCode)) {
        std::cerr << "HashGridRenderer: Failed to read shader files" << std::endl;
        return false;
    }
    ...
    if (createShaderModule(vulkanDevice, vertCode, vertModule) != VK_SUCCESS ||
        ...
        std::cerr << "HashGridRenderer: Failed to create shader modules" << std::endl;
        return false;
    }
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &pipelineLayoutInfo, nullptr,
        ...
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragModule, nullptr);
        std::cerr << "HashGridRenderer: Failed to create pipeline layout" << std::endl;
        return false;
    }
    ...
    if (vkCreateGraphicsPipelines(vulkanDevice.getDevice(), VK_NULL_HANDLE, 1,
        ...
        vkDestroyShaderModule(vulkanDevice.getDevice(), vertModule, nullptr);
        vkDestroyShaderModule(vulkanDevice.getDevice(), fragModule, nullptr);
        std::cerr << "HashGridRenderer: Failed to create graphics pipeline" << std::endl;
        return false;
    }
...
bool HashGridRenderer::allocateDescriptorSetsForModel(uint32_t runtimeModelId, uint32_t maxFramesInFlight) {
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo,
        ...
        std::cerr << "HashGridRenderer: Failed to allocate descriptor sets for model" << std::endl;
        return false;
    }
