    ...
    double remaining, GeodesicTraceResult& baseResult, double totalLength) const
{
    ...
    double minCross = 1e9;
    constexpr double WEDGE_EPS = 1e-8;
    uint32_t minCrossHe = HalfEdgeMesh::INVALID_INDEX;

    int guard = 0;
    do {
        // Standardized CCW traversal around the vertex
    ...
    do {
        ...
        probe = conn.getNextAroundVertex(probe);
        if (probe == HalfEdgeMesh::INVALID_INDEX) {
            isBoundaryVertex = true; 
            break; 
        }
    ...
    // baseResult already has the vertex point as its last pathPoint
    ...
    for (int iter = 0; iter < options.maxIters && remaining > EPS_REMAIN; ++iter) {
        ...
        if (step.hitVertex) {
            ...
            if (inHe == HalfEdgeMesh::INVALID_INDEX || halfEdges[inHe].opposite == HalfEdgeMesh::INVALID_INDEX) {
                return traceFromVertex(step.vertexIdx, glm::normalize(dir2D), remaining, result, length);
            }
            ...
            return traceFromVertex(step.vertexIdx, dirVertex, remaining, result, length);
        }

        // Handle inside face (trace ended within face, no edge hit)
        if (!step.hitEdge) {
            if (remaining <= EPS_REMAIN) {
...
glm::dvec2 GeodesicTracer::rotateVectorAcrossEdge(uint32_t oldHe, uint32_t newHe, const glm::dvec2& vecInOld) const
{
...
GeodesicTracer::GeodesicTraceResult GeodesicTracer::traceFromEdge(uint32_t startEdgeIdx, double startSplit, const glm::dvec2& cartesianDir, double length, uint32_t resolutionFace) const {
    GeodesicTraceResult result;
    result.success = false;
    ...
    if (sourceFace == face1) {
        ...
        if (projIntoFace1 > 0) {
            ...
            dirInTargetFace = rotateVectorAcrossEdge(canonicalHe, oppHe, cartesianDir);
        }
    } else {  
        ...
        if (projIntoFace2 > 0) {
            ...
            targetFace = face1;
            tEdge = startSplit;
            dirInTargetFace = rotateVectorAcrossEdge(oppHe, canonicalHe, cartesianDir);
        }
    }
