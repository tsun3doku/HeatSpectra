...
void GridRenderer::createGridDescriptorPool(uint32_t maxFramesInFlight) {
    ...
    if (vkCreateDescriptorPool(vulkanDevice.getDevice(), &poolInfo, nullptr, &gridDescriptorPool) != VK_SUCCESS) {
        std::cerr << "[GridRenderer] Failed to create descriptor pool" << std::endl;
        return;
    }
}

void GridRenderer::createGridDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(vulkanDevice.getDevice(), &layoutInfo, nullptr, &gridDescriptorSetLayout) != VK_SUCCESS) {
        std::cerr << "[GridRenderer] Failed to create descriptor set layout" << std::endl;
        return;
    }
}

void GridRenderer::createGridDescriptorSets(uint32_t maxFramesInFlight) {
...
VkDescriptorSetAllocateInfo allocInfo{};
    ...
    if (vkAllocateDescriptorSets(vulkanDevice.getDevice(), &allocInfo, gridDescriptorSets.data()) != VK_SUCCESS) {
        std::cerr << "[GridRenderer] Failed to allocate descriptor sets" << std::endl;
        return;
    }
...
void GridRenderer::createGridPipeline(VkRenderPass renderPass, uint32_t subpass) {
    ...
    if (vkCreatePipelineLayout(vulkanDevice.getDevice(), &gridPipelineLayoutInfo, nullptr, &gridPipelineLayout) != VK_SUCCESS) {
        std::cerr << "[GridRenderer] Failed to create pipeline layout" << std::endl;
...
void GridRenderer::cleanup() const {
    if (gridLabel) {
